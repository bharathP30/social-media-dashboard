   ...
   return (
      <div className={`w-full min-h-screen flex items-center justify-center ${theme === 'light' ? 'bg-linear-to-tr from-purple-500 via-pink-400 to-blue-500' : 'bg-linear-to-tr from-black via-gray-900/90 to-black' }`}>
        ...
        <form onSubmit={handleSubmit} 
              className={`px-6 py-10 w-3/4 lg:w-2/5 border flex flex-col justify-center items-center gap-2 rounded-4xl shadow-lg space-y-4 ${theme === 'light' ? 'bg-white/20 text-black border-white/10 backdrop-blur-lg' : 'bg-black/30 backdrop:backdrop-blur-2xl text-white border-gray-400/20' }`}
              >
                
          <h1 className="text-xl font-medium mb-8">Login form</h1>
          
          <input type="text"
                className={`outline-none p-4 w-full md:w-11/12 rounded-md focus:ring-1 ${theme === 'light' ? ' bg-gray-100/50 focus:ring-white/50 text-black' : 'bg-black/30 text-white focus:ring-white/10'}`} 
                name="username"
                value={name}
